# -*- coding: utf-8 -*-
"""Evaluate DeepForest+SAM2 on the merged COCO validation set.

Same protocol as eval_maskrcnn.py: pycocotools COCOeval for Box + Mask mAP.
"""
...
CONF_THR  = 0.30   # same as Mask R-CNN eval
PATCH     = 400
OVERLAP   = 0.05
...
df_model.config["score_thresh"] = 0.05  # low threshold, filter later by CONF_THR
...
# ── Load COCO GT ─────────────────────────────────────────────────────────────
import pycocotools.coco as _coco_mod
_orig_open = open
...
for img_id in img_ids:
    ...
    # Read with PIL to handle Cyrillic paths
...
print("="*60)
print(f"Box  mAP@50   : {stats_box[1]:.3f}")
print(f"Box  mAP@50:95: {stats_box[0]:.3f}")
print(f"Mask mAP@50   : {stats_segm[1]:.3f}")
print(f"Mask mAP@50:95: {stats_segm[0]:.3f}")
print("="*60)
