...
3. A **three-model architecture** combining instance-segmentation (YOLOv8-seg), bounding-box detection (DeepForest fine-tuned) and zero-shot mask refinement (SAM on DeepForest boxes), with a Weighted-Box-Fusion ensemble of the first two branches. The complementarity of the YOLO and DeepForest failure modes — established quantitatively in Section 3.4.3 — is a novel observation in the urban-tree literature.
