...
4. Train and fine-tune four complementary detection models (an instance-segmentation YOLOv8 network, a Mask R-CNN instance segmenter, a DeepForest RetinaNet detector and a SAM 2 zero-shot mask-refinement stage), and combine their detector outputs through a model-ensemble strategy.
...
6. Evaluate the resulting system in terms of detection quality, computational cost and practical applicability, and compare the four model branches against each other on a single dedicated validation set and against published baselines from the literature.
...
- **Deep-learning model training and fine-tuning** based on the PyTorch and Ultralytics frameworks, the DeepForest library, the SAM 2 (Segment Anything 2) foundation model and torchvision/Detectron2 for the Mask R-CNN branch.
...
- It is, to the best of the authors' knowledge, the first published evaluation of state-of-the-art deep-learning tree-detection models on satellite imagery of Astana and, more generally, of any major Central-Asian capital. In particular, Section 3.5.1 reports the first measured magnitude of the geographic-generalisation gap for the region: Box mAP@50 = 0.012 for the public NEON-pretrained DeepForest checkpoint on Astana imagery, three orders of magnitude below the same model's reported off-the-shelf F-score on NAIP USA imagery.
- The work proposes a hybrid four-model architecture (YOLOv8-seg instance segmentation; Mask R-CNN instance segmentation; DeepForest fine-tuned on local data; SAM 2 zero-shot mask refinement of DeepForest bounding boxes), combined through a Weighted-Box-Fusion ensemble of the detector outputs, and quantitatively compares the branches on a single dedicated validation set (Section 3.7).
- It introduces a small but novel annotated dataset of Astana satellite imagery, built iteratively as three batches (v1 / v2 / v3) totalling approximately 100 source images and ≈ 8 700 polygon-level tree annotations after tiling, with a documented per-image annotation-cost reduction from approximately 25 minutes (v1, from scratch) to approximately 4 minutes (v3, model-in-the-loop pre-labelling). The dataset is reusable for future research in the region.
- It reports a **systematic 16-experiment hyperparameter ablation** of the YOLO branch along five orthogonal axes (model size, starting weights, optimiser, augmentation level, inference resolution) — to the best of the authors' knowledge the first such study on a Central-Asian satellite-tree dataset. The ablation establishes the counter-intuitive empirical result that the medium-size YOLOv8m-seg variant (27 million parameters) **outperforms** the larger YOLOv8x-seg (71 M) and YOLOv8l-seg (46 M) variants by 15 – 25 % relative on this dataset scale, and that fresh COCO pre-training beats continuation from a previously fine-tuned checkpoint for the smaller model — both findings are at odds with the "use the largest available model" default and have practical relevance for any project working on similar small annotated satellite corpora.
- It contributes a complete reusable software template — a FastAPI backend with a pluggable four-model adapter interface, a Leaflet-based frontend with in-browser ESRI and Google tile capture (Auto-Zoom Region Scan with streaming progress), SQLite persistent storage of every snapshot and detection, and three exporters (GeoJSON, CSV, standalone HTML) — that turns research models into a deployable internal tool.
...
**Chapter 2** describes the proposed methodology: the overall system architecture, the four deep-learning models (YOLOv8-seg, Mask R-CNN, DeepForest, SAM 2), the data-preparation pipeline (annotation, tiling, augmentation), the ensemble strategy and the geographic-conversion and export components.
