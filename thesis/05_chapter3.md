...
**(5) Continual chain learning hurts on small uniformly-noisy datasets.** A 3-stage continual chain (exp11, v1 → v1+v2 → v1+v2+v3) gave 0.210 vs single-shot 0.308 — a 32 % relative regression. A control experiment with **random splits** of the same training corpus (33 % → 66 % → 100 %, exp17) lost only 0.021 against single-shot, confirming that **77 % of the chain penalty is distribution drift between the v1, v2 and v3 annotation batches**, not the chain mechanism per se. Chain learning is appropriate when there is a clear gradient of label quality along the chain (weak large pre-training, clean small fine-tune); for three uniformly-noisy small batches the optimal strategy is single-shot training on the union of all available data.
...
**(2) Single class label.** The dataset annotation does not distinguish between species. Tree-species classification is a valuable downstream task for *Zelenstroy*, which must plan species-specific pruning and replacement, and is reserved for future work, possibly using a multi-task setup [@Martins2021Species].
