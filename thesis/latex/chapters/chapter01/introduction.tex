...
\subsection{Foundation models: Segment Anything}\label{foundation-models-segment-anything}

The newest paradigm in the field is the \textbf{foundation-model} approach exemplified by the Segment Anything Model (SAM) {[}25{]}. SAM was trained by Meta AI on the SA-1B dataset of more than one billion masks across eleven million images and exposes a \emph{prompt-based} interface in which the user supplies a point, a bounding box or a coarse mask, and the model returns a precise segmentation of the corresponding object.

The critical property of SAM is its \textbf{zero-shot generalisation}: because the model was trained on such a diverse dataset, it produces sharp masks even on object categories that were never explicitly labelled at training time, including trees in satellite imagery. The implication is that SAM can be used as a \textbf{mask-refinement stage} on top of any bounding-box detector --- DeepForest in our case --- without requiring an additional fine-tune. This idea is explored in detail in Section 2.6 of the methodology chapter.
