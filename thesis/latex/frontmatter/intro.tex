...
\item
  Train and fine-tune three complementary detection models (an instance-segmentation YOLOv8 network, a DeepForest RetinaNet detector and a SAM-based mask-refinement stage), and combine their outputs through a model-ensemble strategy.
\item
...
\item
  \textbf{Deep-learning model training and fine-tuning} based on the PyTorch and Ultralytics frameworks, the DeepForest library and the SAM (Segment Anything) foundation model.
\item
...
\textbf{Chapter 2} describes the proposed methodology: the overall system architecture, the three deep-learning models (YOLOv8-seg, DeepForest, SAM), the data-preparation pipeline (annotation, tiling, augmentation), the ensemble strategy and the geographic-conversion and export components.
