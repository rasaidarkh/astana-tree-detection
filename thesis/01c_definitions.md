...
**CIoU loss (Complete IoU loss)** — an extension of the plain IoU loss that, in addition to the box-overlap term, penalises the centre-point distance between predicted and ground-truth boxes and the inconsistency of their aspect ratios. CIoU is the default box-regression loss of YOLOv8 and is used throughout the YOLO branch of the present work.

**COCO (Common Objects in Context)** — the public benchmark dataset and the evaluation protocol it defines. In this work the term refers to the evaluation protocol — `pycocotools.cocoeval.COCOeval` — used to compute Box and Mask mAP@50 / mAP@50:95 on the M14 validation set across all model branches.
...
**Focal loss** — a classification loss introduced by RetinaNet that down-weights the contribution of well-classified examples through a $(1 - p_t)^{\gamma}$ modulating factor, so the gradient is dominated by difficult cases. Focal loss is the classification loss of the DeepForest branch in the present work.
...
**M14 (14-image merged validation set)** — the dedicated cross-model validation set constructed in this work for apples-to-apples comparison between all model branches. It contains 14 source images (4 v1 + 5 v2 + 5 v3) and 702 polygon annotations, generated by `ml/build_14img_val_coco.py` and stored as `yolov train dataset/annotations_merged_14img_val.json`. See Section 3.7.1.

**mAP (mean Average Precision)** — the primary metric for object detection, defined as the mean of per-class Average Precision values; mAP@50 uses an IoU threshold of 0.5 to determine whether a predicted box matches a ground-truth box. mAP@50:95 averages the AP at ten IoU thresholds from 0.50 to 0.95 in increments of 0.05.

**Mask R-CNN** — a two-stage instance-segmentation network that extends the Faster R-CNN detector with a fully-convolutional mask head running in parallel with the bounding-box regression head. In the present work the `maskrcnn_resnet50_fpn_v2` variant is used.
