...
Training images are high-zoom Astana captures, ~0.3 m per pixel, tiled with overlap. Trees touch, so we label polygons, not just boxes. The database is four tables — scans, snapshots, runs, detections — with cascading deletes. Everything trains on a single laptop GPU, an RTX 4060 with 8 GB — on purpose, to prove it's reproducible without a cluster.
...
- *"Dataset size?"* — small, honestly; 14-image / 702-polygon common test set, every engine scored identically.
